...
@click.option(
    ...
    "prefix",
    default=" - ",
    help='Prefix for author(s). Default: " - ".',
)
@click.option("-e", "--epub", "epub", is_flag=True, help="Also rename `.epub`.")
@click.option(
    "-k",
    "--kepub",
    "kepub",
    is_flag=True,
    help="Also rename `.kepub` and `.kepub.epub`.",
)
@click.option(
    "-z", "--zipfile", "zipfile", is_flag=True, help="Also rename `.zip` and `.cbz`."
)
@click.option("-o", "--opf", "opf", is_flag=True, help="Also rename `metadata.opf`.")
@click.option(
    "-g", "--get-cover", "getcover", is_flag=True, help="Get cover image from Amazon."
)
...
def main(paths, prefix, epub, kepub, zipfile, opf, getcover):
    for infile in paths:
        ...
        suffix_bytes = (
            len(("." + asin + prefix + authors + ".kepub.epub").encode()) + 21
        )  # to prevent OSError in Kepubify.
        _MAX_WIDTH = 255
